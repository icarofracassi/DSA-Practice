...
bool vote(int voter, int rank, string name)
{
    for (int i = 0; i < candidate_count; i++) // Iterate over each candidate
    {
        ...
        {
            // preferences[i][j] is jth preference for voter i
            preferences[voter][rank] = i; // if yes, save index of candidate in voter's rank
            return true;
        }
    }
...
void tabulate(void)
{
    ...
    {
        for (int j = 0; j < candidate_count; j++) // for every rank, from bottom up
        {
            int idxCandidate = preferences[i][j];

            if (candidates[idxCandidate].eliminated == false)
            {
                candidates[idxCandidate].votes++;
                break;
            }
        }
    }
